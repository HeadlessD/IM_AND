...
public class NIM_ImageView extends AppCompatImageView {
    ...
    public NIM_ImageView(Context context) {
        this(context, null);
    }

    public NIM_ImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NIM_ImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NIM_ImageView);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.NIM_ImageView_android_src) {
            mDefaultImageId = typedArray.getResourceId(attr, 0);
        } else if (attr == R.styleable.NIM_ImageView_bga_iv_circle) {
            mCircle = typedArray.getBoolean(attr, mCircle);
        } else if (attr == R.styleable.NIM_ImageView_bga_iv_cornerRadius) {
            mCornerRadius = typedArray.getDimensionPixelSize(attr, mCornerRadius);
        } else if (attr == R.styleable.NIM_ImageView_bga_iv_square) {
            mSquare = typedArray.getBoolean(attr, mSquare);
        } else if (attr == R.styleable.NIM_ImageView_bga_iv_borderWidth) {
            mBorderWidth = typedArray.getDimensionPixelSize(attr, mBorderWidth);
        } else if (attr == R.styleable.NIM_ImageView_bga_iv_borderColor) {
