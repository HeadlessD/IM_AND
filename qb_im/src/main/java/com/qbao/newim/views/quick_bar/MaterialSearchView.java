...
import com.qbao.newim.adapter.PhoneSearchAdapter;
import com.qbao.newim.adapter.SearchAdapter;
import com.qbao.newim.adapter.SearchGroupAdapter;
...
import com.qbao.newim.util.AnimationUtil;
...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
    ...
    private void initiateView() {
        LayoutInflater.from(mContext).inflate(R.layout.nim_search_view, this, true);
        mSearchLayout = findViewById(R.id.search_layout);

        mSearchTopBar = (RelativeLayout) mSearchLayout.findViewById(R.id.search_top_bar);
        mSuggestionsListView = (RecyclerView) mSearchLayout.findViewById(R.id.suggestion_list);
        LinearLayoutManager layoutManager = new LinearLayoutManager(mContext, LinearLayoutManager.VERTICAL, false);
        mSuggestionsListView.setLayoutManager(layoutManager);
    ...
    public void showSuggestions() {
        if (mAdapter != null && mAdapter.getItemCount() > 0 && mSuggestionsListView.getVisibility() == GONE) {
     ...
     *
     */
    public void setSuggestions(List<Object> mList) {
        if (mList != null && mList.size() > 0) {
            mTintView.setVisibility(VISIBLE);
            SearchAdapter adapter = new SearchAdapter(mList, new ArrayList<>());
            setAdapter(adapter);

            mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                    setQuery(adapter.getItem(position));
                }
            });
        } else {
    ...
    public void setGroupSuggestions(List<IMGroupInfo> mList) {
        if (mList != null && mList.size() > 0) {
            mTintView.setVisibility(VISIBLE);
            final SearchGroupAdapter adapter = new SearchGroupAdapter(mList, new ArrayList<IMGroupInfo>());
            setAdapter(adapter);
    ...
    public void setPhoneSuggestions(List<Contact> mList) {
        if (mList != null && mList.size() > 0) {
            mTintView.setVisibility(VISIBLE);
            final PhoneSearchAdapter adapter = new PhoneSearchAdapter(mList, new ArrayList<Contact>());
            setAdapter(adapter);
...
//        }
        ...
        if (mOnQueryChangeListener != null) {
            mOnQueryChangeListener.onQueryTextSubmit(object);
            closeSearch();
        }
//        if (submit && !TextUtils.isEmpty(mSearchSrcTextView.getText())) {
//            onSubmitQuery();
//        }
    }
    ...
    public interface OnQueryTextListener {
         ...
         *
         * @param object the query text that is to be submitted
         * @return true if the query has been handled by the listener, false to let the
         * SearchView perform the default action.
         */
        boolean onQueryTextSubmit(Object object);
