...
public class KeyboardUtil {
    ...
    private static boolean saveKeyboardHeight(final Context context, int keyboardHeight) {
        ...
        Logger.debug("KeyBordUtil", String.format("save keyboard: %d", keyboardHeight));

        return SharedPreferenceUtil.saveKeyBoardHeight(keyboardHeight);
    }
    ...
    public static int getKeyboardHeight(final Context context) {
        if (LAST_SAVE_KEYBOARD_HEIGHT == 0) {
            LAST_SAVE_KEYBOARD_HEIGHT = SharedPreferenceUtil.getKeyBoardHeight(getMinPanelHeight(context.getResources()));
        }
    ...
    public static ViewTreeObserver.OnGlobalLayoutListener attach(final Activity activity, IPanelHeightTarget target,
        ...
        final boolean isFullScreen = AppUtil.isFullScreen(activity);
        final boolean isTranslucentStatus = AppUtil.isTranslucentStatus(activity);
        final boolean isFitSystemWindows = AppUtil.isFitsSystemWindows(activity);
    ...
    private static class KeyboardStatusListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        KeyboardStatusListener(boolean isFullScreen, boolean isTranslucentStatus,
            ...
            this.statusBarHeight = StatusbarUtils.getStatusBarHeight(contentView.getContext());
